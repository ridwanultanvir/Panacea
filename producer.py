...
def getAdmittedPatientsList():
    connection = connect()
    cursor = connection.cursor()
    query = '''select PATIENT_ID from ROOM_ADMISSION
...
def get_sensor_data(cond, bed_id):
    ...
    #                               random.randint(heart_beat_h_range[0], heart_beat_h_range[1])])) + " beats/min"
    # sys_blood_pressure = str(random.randint(sys_pres_range[0], sys_pres_range[1])) + " mmHg"
    # dias_blood_pressure = str(random.randint(dias_pres_range[0], dias_pres_range[1])) + " mmHg"

    # body_temp = str(round(random.uniform(body_temp_range[0], body_temp_range[1]), 2)) + " degree F"
    # oxygen_level = str(round(random.uniform(oxygen_level_range[0], oxygen_level_range[1]), 3)) + "%"
    # breathing_rate = str(random.randint(breathing_rate_range[0], breathing_rate_range[1])) + " times/min"

    heartbeat = str(random.choice([random.randint(heart_beat_l_range[0], heart_beat_l_range[1]),
                                   random.randint(heart_beat_h_range[0], heart_beat_h_range[1])]))
    sys_blood_pressure = str(random.randint(
        sys_pres_range[0], sys_pres_range[1]))
    dias_blood_pressure = str(random.randint(
        dias_pres_range[0], dias_pres_range[1]))

    body_temp = str(round(random.uniform(
        body_temp_range[0], body_temp_range[1]), 2))
    oxygen_level = str(round(random.uniform(
        oxygen_level_range[0], oxygen_level_range[1]), 3))
    breathing_rate = str(random.randint(
        breathing_rate_range[0], breathing_rate_range[1]))
