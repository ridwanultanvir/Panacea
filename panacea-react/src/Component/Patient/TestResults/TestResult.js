...
class PatientTestResults extends Component {
    constructor(props) {
        super(props);
        this.state = {
            tests: null
    ...
    fetchData(body) {
        fetch(baseUrl + 'checkup/patient/get-test-results/', {
            method: 'POST',
            headers: {
            ...
            .then((response) => {
                if (response.success) {
                    this.setState({ tests: response.test_results })
                }
                else {
...
export default withStyles(styles)(PatientTestResults);